...
library(xtable)
tblZoneSubZoneRuns <- finalData %>% select(lon, lat, to, km, miles, hours, date, time, elevation) %>% 
         arrange(km, desc(date))

x.big <- xtable(tblZoneSubZoneRuns,
                digits = rep(0, ncol(tblZoneSubZoneRuns)+1),
...
rws <- seq(1, (nrow(tblZoneSubZoneRuns)-1), by = 2)
...
# 
# plot(lake_sf$geometry)
# 
# lake_sp <- sf:::as_Spatial(lake_sf$geometry) 
# 
